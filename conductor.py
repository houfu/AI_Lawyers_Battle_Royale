...
from prompts import Scenario, counsel_template, court_template, costs_system_template, costs_user_template
...
class Conductor:
    def __init__(self, openai_key: str, scenario: Scenario):
    ...
    def run(self):
        ...
        if last_message["role"] == "court":
            if last_message["content"][-4:] == '[PC]':
                ...
                llm = ChatOpenAI(openai_api_key=self.openai_key, streaming=True, callbacks=[stream_handler])
    ...
    def convert_messages_to_langchain_schema(self, messages, role: str):
        system_message_prompt = SystemMessagePromptTemplate.from_template(
            counsel_template if role == 'counsel' else court_template
        )
